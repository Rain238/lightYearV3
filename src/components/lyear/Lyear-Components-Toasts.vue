      ...
      <div class="col-lg-12">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <p>Toast 是轻量级通知，旨在模仿移动和桌面操作系统已经普及的推送通知。它们是用 flexbox
              构建的，因此它们很容易对齐和定位。</p>
            <h6>概述</h6>
            ...
            <h6>基本的</h6>
            <p>为了鼓励可扩展和可预测的 toast，我们建议使用 header 和 body。 Toast 标题使用 <code>display: flex</code>，由于我们的
              <code>margin</code> 和 <code>flexbox</code> 实用程序，可以轻松对齐内容。</p>
            <p>Toast 可以根据您的需要灵活使用，并且所需的标记非常少。至少，我们需要一个元素来包含您的 “toasted”
              内容，并强烈建议使用关闭按钮。</p>
              ...
              <div class="toast" role="alert" aria-live="assertive" aria-atomic="true">
                <div class="toast-header">
                  <svg class="rounded me-2" width="20" height="20" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <rect fill="#33cabb" width="100%" height="100%"></rect>
                  </svg>
                  <strong class="me-auto">Bootstrap</strong> <small>11分钟之前</small>
...
&lt;/div&gt;</pre>
            <div class="callout callout-warning mb-3">
              以前，我们的脚本动态添加了 <code>.hide</code> 类来完全隐藏 toast（使用 <code>display:none</code>，而不仅仅是使用
              <code>opacity:0</code>）。现在不再需要了。但是，为了向后兼容，我们的脚本将继续切换类（即使实际上没有必要），直到下一个主要版本。
            </div>
            <h6>在线示例</h6>
            ...
            <div class="border-example bg-dark">
              <div class="toast" role="alert" aria-live="assertive" aria-atomic="true">
                <div class="toast-header">
                  ...
                  <strong class="me-auto">Bootstrap</strong> <small class="text-muted">11分钟之前</small>
                  <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
                </div>
                <div class="toast-body">
              ...
              <div class="toast-container">
                <div class="toast" role="alert" aria-live="assertive" aria-atomic="true">
                  <div class="toast-header">
                    ...
                    <strong class="me-auto">Bootstrap</strong> <small class="text-muted">刚刚</small>
                    <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
                  </div>
                  <div class="toast-body">
                ...
                <div class="toast" role="alert" aria-live="assertive" aria-atomic="true">
                  <div class="toast-header">
                    ...
                    <strong class="me-auto">Bootstrap</strong> <small class="text-muted">2秒前</small>
                    <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
                  </div>
                  <div class="toast-body">
...
&lt;/div&gt;</pre>
            <h6>自定义内容</h6>
            <p>通过删除子组件、使用实用程序调整它们或添加您自己的标记来自定义您的 toast。在这里，我们通过删除默认的 <code>.toast-header</code>、从
              Bootstrap Icons 添加自定义隐藏图标并使用一些 <code>flexbox</code> 实用程序来调整布局，创建了一个更简单的
              toast。</p>
            <div class="border-example">
...
&lt;/div&gt;</pre>
            <h6>配色方案</h6>
            <p>在上述示例的基础上，您可以使用我们的颜色和背景实用程序创建不同的吐司配色方案。在这里，我们将 <code>.bg-primary</code>
              和 <code>.text-white</code> 添加到 <code>.toast</code>，然后将 <code>.btn-close-white</code>
              添加到关闭按钮。对于清晰的边缘，我们使用 <code>.border-0</code> 删除默认边框。</p>
            <div class="border-example">
...
&lt;/div&gt;</pre>
            <h6>放置</h6>
            <p>根据需要使用自定义 CSS 放置 toast。右上角通常用于通知，顶部中间也是如此。如果您一次只显示一个 toast，请将定位样式放在
              <code>.toast</code> 上。</p>
            <div class="border-example form-floating">
              <form>
                <div class="mb-3"><label for="selectToastPlacement">Toast 放置方式</label> <select
                    class="form-select mt-2" id="selectToastPlacement">
                ...
                <div class="toast-container position-absolute p-3" id="toastPlacement">
                  <div class="toast">
                    <div class="toast-header">
                      ...
                      <strong class="me-auto">Bootstrap</strong> <small>11分钟前</small>
                    </div>
                    <div class="toast-body">
                ...
                <div class="toast-container position-absolute top-0 end-0 p-3">
                  ...
                  <div class="toast" role="alert" aria-live="assertive" aria-atomic="true">
                    <div class="toast-header">
                      ...
                      <strong class="me-auto">Bootstrap</strong> <small class="text-muted">2秒钟之前</small>
                      <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
                    </div>
                    <div class="toast-body">
...
&lt;/div&gt;</pre>
            <h6>可访问性</h6>
            <p>Toast 旨在对您的访问者或用户造成小干扰，因此为了帮助那些使用屏幕阅读器和类似辅助技术的人，您应该将 Toast
              包装在 aria-live 区域中。屏幕阅读器会自动宣布对活动区域的更改（例如注入/更新 toast
              组件），而无需移动用户的焦点或以其他方式打断用户。此外，包括 <code>aria-atomic="true"</code> 以确保始终将整个
              toast 宣布为单个（原子）单元，而不是仅仅宣布更改的内容（如果您只更新 toast 的部分内容，这可能会导致问题，或者如果稍后显示相同的
              toast 内容）。如果所需的信息对流程很重要，例如对于表单中的错误列表，然后使用警报组件而不是 toast。</p>
            <p>请注意，在生成或更新 toast
              之前，活动区域需要存在于标记中。如果同时动态生成两者并注入到页面中，一般不会被辅助技术公布。</p>
            <p>您还需要根据内容调整 <code>role</code> 和 <code>aria-live</code> 级别。如果是错误等重要消息，请使用 <code>role="alert"</code>
              <code>aria-live="assertive"</code>，否则使用 <code>role="status"</code> <code>aria-live="polite"</code> 属性。
            </p>
...
&lt;/div&gt;</pre>
            ...
            <div class="border-example">
              <div role="alert" aria-live="assertive" aria-atomic="true" class="toast" data-bs-autohide="false">
                <div class="toast-header"><i class="mdi mdi-information-outline me-2 fs-5 text-info"></i> <strong
                    class="me-auto">消息</strong> <small>11分钟之前</small>
                  <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
                </div>
                <div class="toast-body">
  ...
  &lt;/div&gt;
&lt;/div&gt;</pre>
            <p>虽然从技术上讲，可以在 toast 中添加可聚焦/可操作的控件（例如附加按钮或链接），但您应该避免为自动隐藏 toast
              执行此操作。即使您为 toast 设置了较长的延迟超时时间，键盘和辅助技术用户也可能会发现很难及时到达 toast
              以采取行动（因为 toast 在显示时没有获得焦点）。如果您绝对必须有进一步的控制，我们建议使用带有 <code>autohide:
                false</code> 的 toast。</p>
          </div>
        </div>
      </div>
      <div class="col-lg-12">
...
var toastList = toastElList.map(function (toastEl) {
  return new bootstrap.Toast(toastEl, option)
            ...
            <h6>选项</h6>
            <p>选项可以通过数据属性或 JavaScript 传递。对于数据属性，将选项名称附加到 <code>data-bs-</code>，如 <code>data-bs-animation=""</code>。
            </p>
            <table class="table">
              <thead>
              <tr>
            ...
            <div class="callout callout-danger mb-3"><strong>异步方法和转换</strong>
              <p>所有 API
                方法都是异步的并开始一个转换。他们在转换开始但在结束之前立即返回给调用者。此外，过渡组件上的方法调用将被忽略。</p>
            </div>
            <p><strong>show</strong></p>
            <p>显示元素的 toast。在 toast 实际显示之前（即在 <code>shown.bs.toast</code> 事件发生之前）返回给调用者。你必须手动调用这个方法，而不是你的
              toast 不会显示。</p>
            <pre>toast.show()</pre>
            <p><strong>hide</strong></p>
            <p>隐藏元素的 toast。在 toast 实际被隐藏之前（即在 <code>hidden.bs.toast</code> 事件发生之前）返回给调用者。如果您将自动隐藏设置为
              <code>false</code>，则必须手动调用此方法。</p>
            <pre>toast.hide()</pre>
