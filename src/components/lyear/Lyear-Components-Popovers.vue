...
var popoverList = popoverTriggerList.map(function (popoverTriggerEl) {
  ...
  container: 'body'
    ...
    <h6>示例</h6>
    <div class="border-example"><button type="button" class="btn btn-lg btn-danger" data-bs-toggle="popover" title="《因话录》" data-bs-content="唐代汉族文言笔记小说集。本书所记皆唐代事。">单击切换popover</button>
    </div>
    ...
    <div class="border-example"><button type="button" class="btn btn-secondary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="top" data-bs-content="《三国志》"> Popover on 顶部 </button> <button type="button" class="btn btn-secondary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="right" data-bs-content="清代长篇章回体英雄传奇小说，共68回，简称《说唐》。著者如莲居士。"> Popover on 右侧 </button> <button type="button" class="btn btn-secondary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="bottom" data-bs-content="《大唐狄公案》是一部描写中国人、中国事、播扬中国古老历史和悠久文化传统、长中国人志气的中国公案小说，在中国与世界文化交流史上留下重重的一笔。"> Popover on 底部 </button> <button type="button" class="btn btn-secondary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="left" data-bs-content="《史记》是西汉著名史学家司马迁撰写的一部纪传体史书，是中国历史上第一部纪传体通史，被列为“二十四史”之首。"> Popover on 左侧 </button>
    </div>
...
&lt;/button&gt;</pre>
    ...
    <div class="callout callout-danger"><strong>下次单击时关闭所需的特定标记</strong>
     <p>对于正确的跨浏览器和跨平台行为，您必须使用 <code>&lt;a&gt;</code> 标记，而不是 <code>&lt;button&gt;</code> 标记，并且还必须包含 <code>tabindex</code> 属性。</p>
    </div>
    <div class="border-example"><a tabindex="0" class="btn btn-lg btn-danger" role="button" data-bs-toggle="popover" data-bs-trigger="focus" title="左传" data-bs-content="《春秋左氏传》原名《左氏春秋》，汉朝时又名《春秋左氏》、《春秋内传》，汉朝以后才多称《左传》。">下次点击时关闭</a>
    </div>
  ...
  trigger: 'focus'
    ...
    <div class="border-example"><span class="d-inline-block" tabindex="0" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-content="桃花扇"> <button class="btn btn-primary" type="button" disabled>禁用的按钮</button> </span>
    </div>
...
var popover = new bootstrap.Popover(exampleEl, options)</pre>
    <div class="callout callout-warning mb-3"><strong>使弹出框适用于键盘和辅助技术用户</strong>
    ...
    <div class="callout callout-warning mb-3">
     请注意，出于安全原因，无法使用数据属性提供 <code>sanitize</code>、<code>sanitizeFn</code> 和 <code>allowList</code> 选项。
    </div>
    <table class="table">
     <thead>
      <tr>
     ...
     <tbody>
      ...
      <tr>
       ...
       <td><p>将弹出框附加到特定元素。 示例：<code>container: 'body'</code>。 此选项特别有用，因为它允许您将弹出框定位在文档流中靠近触发元素的位置 - 这将防止弹出框在窗口调整大小期间从触发元素浮动。</p></td>
      </tr>
      <tr>
       <td><code>content</code></td>
       <td>string | element | function</td>
       <td><code>''</code></td>
       <td><p>如果 <code>data-bs-content</code> 属性不存在，则默认内容值。</p><p>如果给定了一个函数，它将被调用，并将其 <code>this</code> 引用设置为弹出框附加到的元素。</p></td>
      </tr>
      <tr>
       <td><code>delay</code></td>
       <td>number | object</td>
       <td><code>0</code></td>
       <td><p>延迟显示和隐藏弹出框（毫秒） - 不适用于手动触发类型</p><p>如果提供了一个数字，延迟将应用于隐藏/显示</p><p>对象结构为：<code>delay: { "show": 500, "hide": 100 }</code></p></td>
      </tr>
      <tr>
      ...
      <tr>
       ...
       <td><p>如何定位弹出框 - auto | top | bottom | left | right.<br>
         当 <code>auto</code> 被指定时，它会动态地重新定向弹出框。</p><p>当一个函数用于确定放置时，它会以弹出框 DOM 节点作为其第一个参数，触发元素 DOM 节点作为其第二个参数来调用。 <code>this</code> 上下文设置为弹出框实例。</p></td>
      </tr>
      <tr>
      ...
      <tr>
       ...
       <td><p>创建弹出框时要使用的基本 HTML。</p><p>弹出框的 <code>title</code> 将被注入到 <code>.popover-header</code> 中。</p><p>弹出框的 <code>content</code> 将被注入到 <code>.popover-body</code> 中。</p><p><code>.popover-arrow</code> 将成为弹出框的箭头。</p><p>最外层的包装元素应该有 <code>.popover</code> 类。</p></td>
      </tr>
      <tr>
       <td><code>title</code></td>
       <td>string | element | function</td>
       <td><code>''</code></td>
       <td><p>如果 <code>title</code> 属性不存在，则默认标题值。</p><p>如果给定了一个函数，它将被调用，并将其 <code>this</code> 引用设置为弹出框附加到的元素。</p></td>
      </tr>
      <tr>
      ...
      <tr>
       ...
       <td>通过提供数组中的展示位置列表（按优先顺序）来定义后备展示位置。 有关更多信息，请参阅 Popper 的 <a href="https://popper.js.org/docs/v2/modifiers/flip/#fallbackplacements" target="_blank">行为文档</a></td>
      </tr>
      <tr>
      ...
      <tr>
       ...
       <td>string | function</td>
       <td><code>''</code></td>
       <td><p>显示时将类添加到弹出框。 请注意，除了模板中指定的任何类之外，还将添加这些类。 要添加多个类，请用空格分隔它们：<code>'class-1 class-2'</code>。</p><p>您还可以传递一个函数，该函数应返回包含其他类名的单个字符串。</p></td>
      </tr>
      <tr>
      ...
      <tr>
       ...
       <td><code>[0, 8]</code></td>
       <td><p>弹出框相对于其目标的偏移量。 您可以使用逗号分隔值在数据属性中传递字符串，例如：<code>data-bs-offset="10,20"</code></p><p>当一个函数用于确定偏移量时，调用它时会使用一个包含 popper 位置、引用和 popper rects 作为其第一个参数的对象。 触发元素 DOM 节点作为第二个参数传递。 该函数必须返回一个包含两个数字的数组：<code>[<a href="https://popper.js.org/docs/v2/modifiers/offset/#skidding-1" target="_blank">skidding</a>, &lt; a href="https://popper.js.org/docs/v2/modifiers/offset/#distance-1" target="_blank"&gt;距离]</code>。</p><p>有关详细信息，请参阅 Popper 的 <a href="https://popper.js.org/docs/v2/modifiers/offset/#options" target="_blank">偏移文档</a>。</p></td>
      </tr>
      <tr>
       <td><code>popperConfig</code></td>
       <td>null | object | function</td>
       <td><code>null</code></td>
       <td><p>要更改 Bootstrap 的默认 Popper 配置，请参阅 <a href="https://popper.js.org/docs/v2/constructors/#options" target="_blank">Popper 的配置</a>。</p><p>当一个函数用于创建 Popper 配置时，它会被一个包含 Bootstrap 的默认 Popper 配置的对象调用。 它可以帮助您使用默认设置并将其与您自己的配置合并。 该函数必须为 Popper 返回一个配置对象。</p></td>
      </tr>
     </tbody>
    </table>
    <div class="callout callout-info mb-3"><strong>单个弹出框的数据属性</strong>
  ...
  '.popover-body': 'another content'
    ...
    <div class="callout callout-info mb-3"><code>setContent</code> 方法接受一个 <code>object</code> 参数，其中每个属性键是弹出框模板中的有效 <code>string</code> 选择器，每个相关的属性值 可以是 <code>string</code> | <code>element</code> | <code>function</code> | <code>null</code>。
    </div>
    <h6>toggleEnabled</h6>
    <p>切换显示或隐藏元素弹出框的能力。</p>
...
<script>
export default {
name: "Lyear-Components-Popovers"
}
</script>
