  ...
  <div class="card">
   ...
   <div class="card-body">
    ...
    <div class="border-example">
     <svg xmlns="http://www.w3.org/2000/svg" style="display: none;"><symbol id="check-circle-fill" fill="currentColor" viewbox="0 0 16 16">
...
&lt;/div&gt;</pre>
    ...
    <div class="alert alert-danger d-flex align-items-center" role="alert"><i class="mdi mdi-alert me-2"></i>
     <div>
      第44回 金毛犼爱财设巧计 山西雁贪功坠牢笼
     </div>
    </div>
    <p>可以使用 <code>fs-2</code> 这样的通用 <code>class</code> 设置字体图标的大小。</p>
    <div class="alert alert-danger d-flex align-items-center" role="alert"><i class="mdi mdi-alert me-2 fs-2"></i>
     <div>
    ...
    <div class="alert alert-warning alert-dismissible fade show" role="alert"><strong>第46回</strong> 入破庙人鬼乱闹 奔古寺差解同行 <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
    <div class="callout callout-warning">
     当警告框（alert）被关闭后，该元素将从页面结构中被删除。如果使用键盘的用户通过关闭按钮关闭了警告框（alert），那么将在页面上丢失焦点并被重置为页面/文档的起始位置（该行为取决于浏览器）。出于这个原因，我们建议你添加额外的 JavaScript 代码来监听 <code>closed.bs.alert</code> 事件，并通过代码调用 <code>focus()</code> 将焦点设置为页面中最合适的位置。如果你打算将焦点移到通常不会获取焦点的元素上的话，请确保为该元素添加了 <code>tabindex="-1"</code> 属性。
    </div>
   </div>
  </div>
 </div>
