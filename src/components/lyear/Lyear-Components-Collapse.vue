...
&lt;/div&gt;</pre>
    ...
    <div class="callout callout-info mb-3">
     请注意，虽然下面的示例设置了 <code>min-height</code> 以避免在我们的文档中过度重绘，但这并不是明确要求的。 <strong>只需要子元素上的<code>width</code>。</strong>
    </div>
    <div class="border-example">
     <p><button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#collapseWidthExample" aria-expanded="false" aria-controls="collapseWidthExample"> 切换宽度折叠 </button></p>
...
&lt;/div&gt;</pre>
    <h5>多个目标</h5>
    <p><code>&lt;button&gt;</code> 或 <code>&lt;a&gt;</code> 可以通过在其 <code>href</code> 或 <code> 中使用选择器引用多个元素来显示和隐藏多个元素 data-bs-target</code> 属性。 多个 <code>&lt;button&gt;</code> 或 <code>&lt;a&gt;</code> 可以显示和隐藏一个元素，如果它们各自用它们的 <code>href</code> 或 <code>data 引用它 -bs-target</code> 属性</p>
    <div class="border-example">
     <p><a class="btn btn-primary" data-bs-toggle="collapse" href="#multiCollapseExample1" role="button" aria-expanded="false" aria-controls="multiCollapseExample1">切换第一个元素</a> <button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#multiCollapseExample2" aria-expanded="false" aria-controls="multiCollapseExample2">切换第二个元素</button> <button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target=".multi-collapse" aria-expanded="false" aria-controls="multiCollapseExample1 multiCollapseExample2">切换两个元素</button></p>
     <div class="row">
      <div class="col">
...
&lt;/div&gt;</pre>
    ...
    <h6>通过数据属性</h6>
    <p>只需向元素添加 <code>data-bs-toggle="collapse"</code> 和 <code>data-bs-target</code> 即可自动分配对一个或多个可折叠元素的控制。 <code>data-bs-target</code> 属性接受一个 CSS 选择器来应用折叠。 请务必将类 <code>collapse</code> 添加到可折叠元素。 如果您希望它默认打开，请添加附加类 <code>show</code>。</p>
  ...
  toggle: false
    ...
    <table class="table">
     ...
     <tbody>
      <tr>
       <td><code>toggle</code></td>
       <td>将可折叠元素切换为显示或隐藏。 <strong>在可折叠元素实际显示或隐藏之前返回调用者</strong>（即在<code>shown.bs.collapse</code>或<code>hidden.bs.collapse</code>之前 事件发生）。</td>
      </tr>
      <tr>
      ...
      <tr>
       ...
       <td>允许您获取与 DOM 元素关联的折叠实例的静态方法，您可以像这样使用它：<code>bootstrap.Collapse.getInstance(element)</code></td>
      </tr>
      <tr>
       <td><code>getOrCreateInstance</code></td>
       <td>静态方法，它返回与 DOM 元素关联的折叠实例，或者在未初始化的情况下创建一个新实例。你可以像这样使用它：<code>bootstrap.Collapse.getOrCreateInstance(element)</code></td>
      </tr>
     </tbody>
    </table>
    <h6>事件</h6>
...
<script>
export default {
name: "Lyear-Components-Collapse"
}
</script>
